...
var layout = {
		...
		loadHome: function(){
			...
			$(document).delegate('select.tb', 'change', function(e){
				$(this).parent('li.tb').next('li.tb_opt').hide().load('./controller.php?module=home/table_menu&tb=' + $(this).val()).fadeIn('slow');
			});
		...
		tabs : {
		    ...
		    add: function(opts){
		        ...
		        if (opts.html){
		        	$('#added' + id).html(opts.html);
		        	if (opts.loaded){
		        		opts.loaded($('#added' + id));
		        	}
		        	...
		        	this.tab.find('li a:last').data('state', {obj: opts.obj, method: opts.method, param: opts.param});
		    ...
		    reloadActive: function(){
		    	...
		    	opts = tab.find('li.active a').data('state');

		    	var url = 'controller.php?obj=' + opts.obj + '&method='+ opts.method;

		    	if (opts.param){
		        	if (typeof opts.param === 'string'){
						url += '&' + opts.param;
					} else if($.isPlainObject(opts.param))  {
						URurl += '&' + $.param(opts.param);
					} else if ($.isArray){
						url += '&param[]=' + opts.param.join('&param[]=');
					}
        		}

		    	d_active.html('<img src="./img/arrows-loader.gif" alt="loading..." />').load(url);
