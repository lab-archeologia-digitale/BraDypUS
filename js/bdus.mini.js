...
if(!post){$.get(URLstring,loaded,'json');}else{$.post(URLstring,post,loaded,'json');}},runAndRespond(obj,method,get,post){let getStr=false;if(typeof get==='object'){let get_part=[];for(const[key,value]of Object.entries(get)){if(value){get_part.push(`${key}=${encodeURIComponent(value)}`);}}
getStr=get_part.join('&');}else if(typeof get==='string'){getStr=get;}
core.getJSON(obj,method,getStr,post,data=>{core.message(data.text,data.status);});},open:function(opt,type){if(type==='modal'){layout.dialog.add(opt);}else{layout.tabs.add(opt);}},tr:function(string,args){if(!lang[string]){return string;}else{if(!args){return lang[string];}else{return vsprintf(lang[string],args);}}},message:function(text,type,sticky){$.pnotify({history:false,text:text,type:type?type:false,hide:sticky?false:true,styling:"bootstrap"});}};
