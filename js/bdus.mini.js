...
else if(!id_arr[0]){core.message(core.tr('select_row_to_continue'),'error');}else{tb=div.find('input.curr_tb').val();successFunction(tb,id_arr,$(this));}}}},{text:core.tr('close'),action:'close'}]},'modal');}},'json');},delete_userlink:function(linkId,successFunction){core.getJSON('userlinks_ctrl','deleteUserLink',{"id":linkId},false,function(data){if(data.status=='success'&&successFunction){successFunction(data);}
core.message(data.text,data.status);});},show_userlinks:function(l_el){var l_context=l_el.data('context');var l_tb=l_el.data('tb');var l_id=l_el.data('id');l_el.html('');core.getHTML('userlinks_ctrl','showUserLinks',{"tb":l_tb,"id":l_id,"context":l_context},function(returned_html){l_el.html(returned_html);$(l_el).find('span.userlink_read').on('click',function(){api.record.read($(this).data('tb'),[$(this).data('id')]);});$(l_el).find('span.userlink_delete').click(function(){var $this=$(this),delId=$this.data('id');api.link.delete_userlink(delId,function(){$this.parent('li').remove();});});$(l_el).find('span.userlink_reload').click(function(){api.link.show_userlinks(l_el);});$(l_el).find('span.userlink_add').click(function(){var thisid=$(this).data('id')
var thistb=$(this).data('table');api.link.add_ui(function(tb,arr_id){core.getJSON('userlinks_ctrl','addUserLink',{thistb:thistb,thisid:thisid,tb:tb,id:arr_id},false,function(data){if(data.status==='success'){api.link.show_userlinks(l_el);}
...
var layout={init:function(){$('<div />').attr('id','wrapper').appendTo('body');$(window).on('resize',function(){layout.setSize();});$(window).on('hashchange',function(e){hashActions.map2action();});},loadHome:function(){var html=`<div class="tabbable">
...
this._label.pack();}}}
ticks=null;};function bestFormatString(interval){var fstr;interval=Math.abs(interval);if(interval>=10){fstr='%d';}
...
default:return hash;}}else{return false;}},map2action(){if(this.getHash('mapId')){core.getJSON('saved_queries_ctrl','getById',{"id":this.getHash('mapId')},false,function(data){if(data.status=='success'){core.runMod('geoface',[data.tb,data.text]);}});}else if(this.getHash('queryId')){core.getJSON('saved_queries_ctrl','getById',{"id":this.getHash('queryId')},false,function(data){if(data.status==='success'){api.showResults(data.tb,'type=obj_encoded&obj_encoded='+data.obj_encoded,core.tr('saved_queries')+' ('+data.tb+')');}else{core.message(core.tr('saved_query_does_not_exist',[this.getHash('queryId')]),'error',true);}});}else if(this.getHash('chartId')){core.runMod('chart',['showChart',this.getHash('chartId')]);}else if(this.getHash('readId')){const hash_data=this.getHash('readId');api.record.read(prefix+hash_data.table,[hash_data.id],hash_data.isIdField);}}};