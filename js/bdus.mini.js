...
var layout={init:function(){$('<div />').attr('id','wrapper').appendTo('body');$(window).on('resize',function(){layout.setSize();});$(window).on('hashchange',function(e){layout.hashActions();});},loadHome:function(){var html='<div class="tabbable">'+'<ul class="nav nav-tabs navbar-fixed-top" id="tabs" data-tabs="tabs">'+'<li class="active"><a href="#home">BraDypUS v3</a></li>'+'</ul>'+'<div class="tab-content">'+'<div class="tab-pane active" id="home"></div>'+'</div>'+'</div>';$('#wrapper').html(html);$('<button id="tabs_toggle" type="button" class="btn btn-primary" style="z-index:9999; position:fixed;top:0; right:0">'+'<i class="icon-reorder"></span>'+'</button>').appendTo($('#wrapper')).on('click',function(){$('#tabs').toggle();});$.get('controller.php?module=home/menu',function(data){$.get('./controller.php?module=home/table_menu&tb='+$($.parseHTML(data)).find('select.tb').val(),function(data2){$('#home').html(data);$('#home').find('li.tb_opt').html(data2);});},'html');$(document).delegate('select.tb','change',function(e){$(this).parent('li.tb').next('li.tb_opt').hide().load('./controller.php?module=home/table_menu&tb='+$(this).val()).fadeIn('slow');});$(document).delegate('input.fast_search','keyup',function(e){var code=(e.keyCode?e.keyCode:e.which);if(code==13){core.runMod('search',['fast',$(this).data('table'),$(this).val()]);}});$(document).delegate('input#find-fn','keyup',function(e){var srch=$(this).val().toLowerCase();if(!srch||srch===''){$('ul.searcheable-list li').each(function(i,el){$(el).removeClass('highlight');});return;}
