...
if(!post){$.get(URLstring,loaded,'json');}else{$.post(URLstring,post,loaded,'json');}},open:function(opt,type){if(type==='modal'){layout.dialog.add(opt);}else{layout.tabs.add(opt);}},tr:function(string,args){if(!lang[string]){return string;}else{if(!args){return lang[string];}else{return vsprintf(lang[string],args);}}
...
default:return hash;}}else{return false;}},map2action(){if(this.getHash('mapId')){core.getJSON('saved_queries_ctrl','getById',[this.getHash('mapId')],false,function(data){if(data.status=='success'){core.runMod('geoface',[data.tb,data.text]);}});}else if(this.getHash('queryId')){core.getJSON('saved_queries_ctrl','getById',[this.getHash('queryId')],false,function(data){if(data.status==='success'){api.showResults(data.tb,'type=encoded&q_encoded='+data.text,core.tr('saved_queries')+' ('+data.tb+')');}else{core.message(core.tr('saved_query_does_not_exist',[this.getHash('queryId')]),'error',true);}});}else if(this.getHash('chartId')){core.runMod('chart',['showChart',this.getHash('chartId')]);}else if(this.getHash('readId')){const hash_data=this.getHash('readId');api.record.read(prefix+hash_data.table,[hash_data.id],hash_data.isIdField);}}};