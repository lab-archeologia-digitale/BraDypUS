...
$.ajax({url:script,dataType:"script",suppressErrors:true,success:function(data){window[mod].init.apply(null,args);if(loaded){loaded();}},error:function(jqxhr,settings,exception){core.open({obj:mod,method:'read',title:core.tr(mod)});}});}},loading:'<img src="./img/loader.gif"  alt="loading..." />',getJSON:function(obj,method,get,post,loaded){var URLstring='./controller.php?obj='+obj+'&method='+method;if(get){if(typeof get==='string'){URLstring+='&'+get;}else if($.isPlainObject(get)){URLstring+='&'+$.param(get);}else if($.isArray(get)){URLstring+='&param[]='+get.join('&param[]=');}}
if(!post){$.get(URLstring,loaded,'json');}else{$.post(URLstring,post,loaded,'json');}},open:function(opt,type){if(type==='modal'){layout.dialog.add(opt);}else{layout.tabs.add(opt);}},getHash:function(what){var hash=location.hash;if(hash.length!==0){hash=hash.substring(2).split('/');switch(what){case'app':return hash[0];case'mapId':if(hash[0]&&hash[1]=='map'&&hash[2]){return hash[2];}else{return false;}
...
break;default:return hash;}}else{return false;}},tr:function(string,args){if(!lang[string]){return string;}else{if(!args){return lang[string];}else{return vsprintf(lang[string],args);}}
...
core.message(data.text,data.status);});},show_userlinks:function(l_el){var l_context=l_el.data('context'),l_tb=l_el.data('tb'),l_id=l_el.data('id');l_el.html('').load('controller.php?obj=userlinks_ctrl&method=show&param[]='+l_tb+'&param[]='+l_id+'&param[]='+l_context,function(){$(l_el).find('span.userlink_read').on('click',function(){api.record.read($(this).data('tb'),[$(this).data('id')]);});$(l_el).find('span.userlink_delete').click(function(){var $this=$(this),delId=$this.data('id');api.link.delete_userlink(delId,function(){$this.parent('li').remove();});});$(l_el).find('span.userlink_reload').click(function(){api.link.show_userlinks(l_el);});$(l_el).find('span.userlink_add').click(function(){var thisid=$(this).data('id'),thistb=$(this).data('table');api.link.add_ui(function(tb,arr_id){core.getJSON('userlinks_ctrl','link',false,{thistb:thistb,thisid:thisid,tb:tb,id:arr_id},function(data){if(data.status=='success'){api.link.show_userlinks(l_el);}
...
var layout={init:function(){$('<div />').attr('id','wrapper').appendTo('body');$(window).on('resize',function(){layout.setSize();});$(window).on('hashchange',function(e){layout.hashActions();});},loadHome:function(){var html='<div class="tabbable">'+'<ul class="nav nav-tabs navbar-fixed-top" id="tabs" data-tabs="tabs">'+'<li class="active"><a href="#home">BraDypUS v3</a></li>'+'</ul>'+'<div class="tab-content">'+'<div class="tab-pane active" id="home"></div>'+'</div>'+'</div>';$('#wrapper').html(html);$('<button id="tabs_toggle" type="button" class="btn btn-primary" style="z-index:9999; position:fixed;top:0; right:0">'+'<i class="icon-reorder"></span>'+'</button>').appendTo($('#wrapper')).on('click',function(){$('#tabs').toggle();});$.get('controller.php?module=home/menu',function(data){$.get('./controller.php?module=home/table_menu&tb='+$($.parseHTML(data)).find('select.tb').val(),function(data2){$('#home').html(data);$('#home').find('li.tb_opt').html(data2);});},'html');$(document).delegate('select.tb','change',function(e){$(this).parents('li.tb').next('li.tb_opt').hide().load('./controller.php?module=home/table_menu&tb='+$(this).val(),function(){$(this).fadeIn('slow');});});$(document).delegate('input.fast_search','keyup',function(e){var code=(e.keyCode?e.keyCode:e.which);if(code==13){core.runMod('search',['fast',$(this).data('table'),$(this).val()]);}});$(document).delegate('input#find-fn','keyup',function(e){var srch=$(this).val().toLowerCase();if(!srch||srch===''){$('ul.searcheable-list li').each(function(i,el){$(el).removeClass('highlight');});return;}
...
return{width:'100%',tags:(typeof tags!=='undefined'),createTag:function(params){return{id:params.term,val:params.term,newOption:true};},ajax:{url:'controller.php?obj=menuValues_ctrl&method=getValuesUrl&context='+context+'&att='+att,dataType:'json',delay:250,tokenSeparators:[';'],data:function(p){return{q:p.term,p:p.page};},processResults:function(data,p){data.data.unshift({"id":'',"val":''}),p.page=p.page||1;return{results:data.data,pagination:{more:(p.page*30)<data.tot}};},cache:true},escapeMarkup:function(markup){return markup;},templateResult:function(a){return a.val;},templateSelection:function(a){return a.hasOwnProperty('val')?a.val:a.text;},minimumInputLength:0};},multiselect:function(el,destroy)
