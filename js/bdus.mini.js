...
return{tags:(typeof tags!=='undefined'),createTag:function(params){return{id:params.term,val:params.term,newOption:true}},ajax:{url:'controller.php?obj=menuValues_ctrl&method=getValuesUrl&context='+context+'&att='+att,dataType:'json',delay:250,tokenSeparators:[';'],data:function(p){return{q:p.term,p:p.page};},processResults:function(data,p){data.data.unshift({"id":'',"val":''}),p.page=p.page||1;return{results:data.data,pagination:{more:(p.page*30)<data.tot}};},cache:true},escapeMarkup:function(markup){return markup;},templateResult:function(a){return a.val;},templateSelection:function(a){return a.hasOwnProperty('val')?a.val:a.text;},minimumInputLength:0}},multiselect:function(el,destroy)
...
{if($(el).data('select2')&&destroy){$(el).datepicker('remove');}else{$(el).datepicker({format:enhance.dateFormat});}}
