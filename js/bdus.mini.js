...
+(get?'&'+this._get2url(get):'');if(!post){$.get(URLstring,loaded,'json');}else{$.post(URLstring,post,loaded,'json');}},runAndRespond(obj,method,get,post){core.getJSON(obj,method,get,post,data=>{core.message(data.text,data.status);});},open:function(opt,type){if(type==='modal'){layout.dialog.add(opt);}else{layout.tabs.add(opt);}},tr:function(string,args){if(!lang[string]){return string;}else{if(!args){return lang[string];}else{return vsprintf(lang[string],args);}}},message:function(text,type,sticky){$.pnotify({history:false,text:text,type:type?type:false,hide:sticky?false:true,styling:"bootstrap"});}};
