...
if(typeof window[mod]==='object'&&!debugMode){window[mod].init.apply(null,args);}else{var script;if(debugMode){script='./modules/'+mod+'/'+mod+'.js';}else{script='./modules/'+mod+'/'+mod+'.min.js';}
...
core.message(data.text,data.status);});},show_userlinks:function(l_el){var l_context=l_el.data('context');var l_tb=l_el.data('tb');var l_id=l_el.data('id');l_el.html('');core.getHTML('userlinks_ctrl','showUserLinks',{"tb":l_tb,"id":l_id,"context":l_context},function(returned_html){l_el.html(returned_html);$(l_el).find('span.userlink_read').on('click',function(){api.record.read($(this).data('tb'),[$(this).data('id')]);});$(l_el).find('span.userlink_delete').click(function(){var $this=$(this),delId=$this.data('id');api.link.delete_userlink(delId,function(){$this.parent('li').remove();});});$(l_el).find('span.userlink_reload').on('click',()=>{api.link.show_userlinks(l_el);});$(l_el).find('span.userlink_add').on('click',(e)=>{var thisid=$(e.target).data('id')
var thistb=$(e.target).data('table');api.link.add_ui(function(tb,arr_id){core.getJSON('userlinks_ctrl','addUserLink',{thistb:thistb,thisid:thisid,tb:tb,id:arr_id},false,function(data){if(data.status==='success'){api.link.show_userlinks(l_el);}
core.message(data.text,data.status);});});});});}},fileUpload:function(el,url,opts){opts=opts||{};if(!el||!url){return false;}
var d={request:{endpoint:url},autoUpload:true,text:{uploadButton:'<div><i class="fa fa-upload"></i> '+(opts.button_text?opts.button_text:core.tr('click_drag_to_upload'))+'</div>',dragZone:core.tr('drop_to_upload')},template:`<div class="qq-uploader">
...
var layout={init:function(){$('<div />').attr('id','wrapper').appendTo('body');$(window).on('resize',function(){layout.setSize();});$(window).on('hashchange',function(e){hashActions.map2action();});},loadHome:function(){var html=`<div class="tabbable">
        ...
        </button>`).appendTo($('#wrapper')).on('click',function(){$('#tabs').toggle();});$.get('./?obj=home_ctrl&method=main_home',function(data){$('#home').html(data);},'html');$(document).on('keyup','input.fast_search',function(e){var code=(e.keyCode?e.keyCode:e.which);if(code==13){core.runMod('search',['fast',$('#ref_tb').val(),$(this).val()]);}});$(document).on('keyup','input#find-fn',function(e){var srch=$(this).val().toLowerCase();if(!srch||srch===''){$('ul.searcheable-list li').each(function(i,el){$(el).removeClass('highlight');});return;}
...
const enhance={getData:function(el){const datalist=$('datalist[for="'+el.attr('id')+'"]').first();let data=[];datalist.children("option").map(function(){const text=$(this).text();const value=$(this).val()?$(this).val():text;data.push({id:value,text:text});});return data;},getAjaxData:el=>{const context=el.data('context');const att=el.data('att');const tags=el.data('tags');if(typeof context==='undefined'||typeof att==='undefined'){return{};}
return{width:'100%',tags:(typeof tags!=='undefined'),createTag:function(params){return{id:params.term,val:params.term,newOption:true};},ajax:{url:'./?obj=menuValues_ctrl&method=getValuesUrl&context='+context+'&att='+att,dataType:'json',delay:250,tokenSeparators:[';'],data:function(p){return{q:p.term,p:p.page};},processResults:function(data,p){data.data.unshift({"id":'',"val":''}),p.page=p.page||1;return{results:data.data,pagination:{more:(p.page*30)<data.tot}};},cache:true},escapeMarkup:markup=>markup,templateResult:a=>{return a.val;},templateSelection:a=>{return a.hasOwnProperty('val')?a.val:a.text;},minimumInputLength:0};},multiselect:(el,destroy)=>{if($(el).data('select2')&&destroy){$(el).select2('destroy');}else{$(el).select2($.extend({},{separator:';',tokenSeparators:[';']},enhance.getAjaxData($(el)))).on('change',function(){$(this).attr('changed','auto');});}},combobox:(el,destroy)=>{if($(el).data('select2')&&destroy){$(el).select2('destroy');}else{$(el).select2($.extend({},{data:enhance.getData($(el),true)},enhance.getAjaxData($(el)))).on('change',function(){$(this).attr('changed','auto');});}},select:(el,destroy)=>{if($(el).data('select2')&&destroy){$(el).select2('destroy');}else{$(el).select2(enhance.getAjaxData($(el))).on('change',function(){$(this).attr('changed','auto');});}},pimpEl:(el,destroy)=>{if($(el).hasClass('combobox')){enhance.combobox(el,destroy);}else if($(el).hasClass('multiselect')){enhance.multiselect(el,destroy);}else if($(el).is('select')){enhance.select(el,destroy);}},form:form=>{$(form).find(':input').each(function(i,el){enhance.pimpEl(el);});}};

/* jquery.checklabel.js */

(function($){$.fn.checklabel=function(){this.each(function(){const label=$(this).find('label');const input=$(this).find('input');input.hide();label.on('click',function(){if(input.is(':checked')){input.attr('checked',false);label.removeClass('selected');}else{input.attr('checked',true);label.addClass('selected');}});});};})(jQuery);
