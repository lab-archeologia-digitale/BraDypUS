...
+(get?'&'+this._get2url(get):'');if(!post){$.get(URLstring,loaded,'json');}else{$.post(URLstring,post,loaded,'json');}},runAndRespond(obj,method,get,post,loaded){core.getJSON(obj,method,get,post,data=>{core.message(data.text,data.status);if(loaded){loaded(data);}});},open:function(opt,type){if(type==='modal'){layout.dialog.add(opt);}else{layout.tabs.add(opt);}},tr:function(string,args){if(!lang[string]){return string;}else{if(!args){return lang[string];}else{return vsprintf(lang[string],args);}}},message:function(text,type,sticky){if(!type){type='info';}
