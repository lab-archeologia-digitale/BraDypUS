...
var api={requireRestart:function(message){var html=(message?'<h3>'+message+'</h3>':'')+'<p class="lead text-warning"><i class="glyphicon glyphicon-warning-sign"></i> '+'<strong>'+core.tr('attention')+'</strong><br />'+core.tr('reload_sys_required')+'</p>';core.open({html:html,title:core.tr('attention'),buttons:[{text:core.tr('restart'),click:function(){api.reloadApp();}},{text:core.tr('cancel'),action:'close'}]},'modal');},confirmSuperAdmPwd:function(success){core.open({title:core.tr('cfg_security_check'),html:`<p class="lead text-danger">
        ...
        </form>`,loaded:()=>{setTimeout(()=>{$('#modal input.adm_pwd').focus();},500);$('#modal form').on('submit',()=>{const pwd=$('#modal .adm_pwd').val();core.getJSON('confirm_super_adm_pwd_ctrl','check_pwd',false,{pwd:pwd},data=>{core.message(data.text,data.status)
