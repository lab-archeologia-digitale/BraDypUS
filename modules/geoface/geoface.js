...
var geoface = {
		...
		addXY: function(x, y, layerEPSG, mapEPSG){
			...
			if (layerEPSG != mapEPSG){
				location = new OpenLayers.LonLat(x, y).transform(  new OpenLayers.Projection("EPSG:" + layerEPSG), new OpenLayers.Projection("EPSG:" + mapEPSG));
			} else {
				location = {
						lon: x,
						lat: y
		...
		showmap: function(){
			...
			var base, geojson_format, vector_layer;
			...
			// AWMS
			var awms = new OpenLayers.Layer.XYZ("AWMC Background",[
			...
			// Imperium
			var imperium = new OpenLayers.Layer.XYZ("Roman Empire",[
			...
			if (typeof(base) != 'undefined'){
				vector_layer = new OpenLayers.Layer.Vector(geoface.metadata.tb);
			} else {
				vector_layer = new OpenLayers.Layer.Vector(geoface.metadata.tb, {isBaseLayer: true});
			}
