...
class sys_mail_ctrl extends Controller
{
	...
	public function send()
	{
		...
		$body = $this->post['body'] . tr::sget('automatic_email_signature', strtoupper(PREFIX));
		
		$headers = 'From: ' . $this->post['from']  . "\r\n" . 'Reply-To: ' . $this->post['from'] . "\r\n";
		
		$users_email = DB::start()->query('SELECT * FROM `' . PREFIX . '__users` WHERE `privilege` IN (' . implode(', ', $this->post['to']) . ')');
	...
	public function showForm()
	{
		...
		echo $this->render('sys_mail', 'form', array(
				'tr' => new tr(),
				'adm_email' => PREFIX . '_admin@bradypus.net',
